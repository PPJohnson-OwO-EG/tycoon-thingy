...
def selection():                                        #selection menu that shows up every 7 days.
    if time_stuff["selection mode"] == 1:
        ...
        while True:
            game_print('Build, Research, Continue')
...
def build():                         #new build menu, much more modular and efficient
    while True:
        ...
        if build_choice in available_buildings:
            print()
            game_print(f'{building[build_choice]["description"]}   Costs: {building[build_choice]["wood cost"]} wood | {building[build_choice]["gold cost"]} gold | {building[build_choice]["stone cost"]} stone | {building[build_choice]["maintenance"]} gold per week in maintenance')
...
researchables = ["Irrigation", "Better Pickaxes", "Wells", "Basic Healthcare", "Mass Scale Mines", "Taxation"]
researched = []
